...
I want to slowly move this monitoring tool from "its down" to "its going down" territory, from an end user point of view.  Ive seen so many monitoring solutions that show a device up when the help line is swamped with people complaining why somethings soooo slow or not working at all. Changes in response time can be a good indicator of something being wrong.  So the next things I want to add are time based checks that can gated with classic checks.
